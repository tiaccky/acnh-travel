...
// 🌟 核心修復：將 paidBy 擴充支援 'Shared' 及 null，解決所有 Vercel 型別報錯
export interface Activity { id: string; time: string; type: string; location: string; address?: string; note?: string; cost: number; currency: string; costHKD: number; paidBy?: 'Big' | 'Small' | 'Shared' | null; isShared: boolean; ratingBig: number; ratingSmall: number; isSurprise?: boolean; paymentMethod?: 'cash' | 'card' | null; }
...
export const useTripStore = create<TripStore>()(
  persist(
    (set) => ({
      ...
      // 🌟 補回客製化貼圖的狀態與方法
      customStickers: [],
      addCustomSticker: (url) => set((state) => ({ customStickers: [url, ...(state.customStickers || [])] })),
      removeCustomSticker: (url) => set((state) => ({ customStickers: (state.customStickers || []).filter(u => u !== url) })),
      ...
      initFirebase: () => {
        const unsub = onSnapshot(doc(db, 'appData', 'shared-trip'), (docSnapshot) => {
          if (docSnapshot.exists()) {
            ...
            if (remoteData && remoteData.length > 0) set({ trips: remoteData });
          }
        });
        return unsub;
      },
      fetchExchangeRate: async () => {
        try { const res = await fetch('https://corsproxy.io/?url=' + encodeURIComponent('https://www.google.com/search?q=1+krw+to+hkd'), { cache: 'no-store' }); const html = await res.text(); const match = html.match(/data-exchange-rate="([0-9.]+)"/); if (match && match[1]) { set({ exchangeRate: parseFloat(match[1]) }); } else { const fallback = await fetch('https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/krw.json', { cache: 'no-store' }); const data = await fallback.json(); if (data.krw.hkd) set({ exchangeRate: data.krw.hkd }); } } catch (e) { console.warn("匯率抓取失敗，使用 0.0052"); }
      },
      ...
      addTrip: (newTrip) => set((state) => {
        ...
        const nextTrips = [trip, ...state.trips]; syncToFirebase(nextTrips); return { trips: nextTrips, activeTripId: trip.id, activeDayIndex: 0 };
      }),
